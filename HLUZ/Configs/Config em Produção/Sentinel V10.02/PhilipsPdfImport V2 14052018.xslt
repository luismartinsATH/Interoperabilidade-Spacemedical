...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="urn:dmr-cmf" xmlns:utils="urn:CodeUtils">
  ...
  <xsl:template match="/">
    <Message>
      ...
      <Test>
        <StartDateTime><!-- 2018-04-23T15:02:05 --></StartDateTime>        <!--   <xsl:call-template name="ExtractRecordingDateTime"/> -->
        <RecordingDuration></RecordingDuration>
        <RecorderModel>MAPA Mortara</RecorderModel>
      ...
      <Reports>
        <Report>
          ...
          <TestType>blood pressure</TestType> <!--             <xsl:call-template name="ExtractTestType"/> -->
          <RecordingDateTime></RecordingDateTime>
          <ReportFormat></ReportFormat>
          <ReportType>blood pressure</ReportType> <!-- <xsl:call-template name="ExtractTestType"/> -->
 ...
 <!--  [LM] Alteração de função que reconhece ID -->
  <xsl:template name="ExtractPatientID">
    <xsl:variable name="filename">
	 ...
	 <xsl:variable name="filenameWithoutExtension" select="substring-before($filename, '.')"/>
	 <xsl:value-of select="substring-after($filenameWithoutExtension, '_')"/>
   <!--  <xsl:value-of select="substring-before($filename, '_')"/>  --> <!-- LMM 14052018 Alteração para extrair ID de ORG_ID -->
  </xsl:template>
  
  <xsl:template name="ExtractORG">
    <xsl:variable name="filename">
      <xsl:call-template name="GetReportFilename"/>
    </xsl:variable>
    <xsl:value-of select="substring-before($filename, '_')"/>  <!-- LMM 14052018 Alteração para extrair ORG de ORG_ID -->
  </xsl:template>
