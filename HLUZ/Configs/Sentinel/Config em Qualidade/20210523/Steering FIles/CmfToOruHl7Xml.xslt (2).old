...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:cmf="urn:dmr-cmf" xmlns:utils="urn:CodeUtils">
  ...
  <xsl:template match="/">
    <Hl7XmlMessage>
      ...
      <PV1>
       <PV1.2>
		<!--  <xsl:value-of select="/cmf:Message/cmf:Test/cmf:UserDefinedField2Value"/> --> <!-- LMM 29112017 Alteração para patient class -->
        <!-- <xsl:value-of select="/cmf:Message/cmf:Patient/cmf:Visit/cmf:Class"/> -->
		  <xsl:choose> <!-- LMM 26102018 Alteração para patient class -->
			<xsl:when test="/cmf:Message/cmf:Patient/cmf:Visit/cmf:Class">
				<xsl:value-of select="/cmf:Message/cmf:Patient/cmf:Visit/cmf:Class"/>
			</xsl:when>
			<xsl:when test="/cmf:Message/cmf:Test/cmf:UserDefinedField2Value">
				<xsl:value-of select="/cmf:Message/cmf:Test/cmf:UserDefinedField2Value"/>
			</xsl:when>
			<xsl:otherwise>U</xsl:otherwise>
		  </xsl:choose>	 
       </PV1.2>
        <PV1.3>
         <xsl:value-of select="/cmf:Message/cmf:Patient/cmf:Location/cmf:PointOfCare"/> <!-- Alteração para HLUZ [LM] 27102017  -->
<!-- 		<PV1.3.1><xsl:value-of select="/cmf:Message/cmf:Patient/cmf:Location/cmf:PointOfCare"/></PV1.3.1>
		...
		<PV1.10>
		   <!-- <xsl:value-of select="/cmf:Message/cmf:Patient/cmf:Location/cmf:Ward"/> --> <!-- Alteração para HLUZ [LM] 27102017  -->
        </PV1.10>
      ...
      <OBR>
        ...
        <OBR.3><xsl:value-of select="/cmf:Message/cmf:Orders/cmf:Order/cmf:FillerNumber"/></OBR.3>
	    	<OBR.4>
          <!-- Devolve o cód de exame original do HIS, armazenado em userfield3 ou em PROCEDURE [LMM] 18022021 -->
          <xsl:call-template name="FunctionESCOLHE_HLUZ"> 
            <xsl:with-param name="campo" select="/cmf:Message/cmf:Test/cmf:UserDefinedField3Value"/>      
            <xsl:with-param name="procedure1" select="/cmf:Message/cmf:Orders/cmf:Order/cmf:Procedure"/>
            <xsl:with-param name="Delimitador" select="'&#46;'"/>
            <xsl:with-param name="Indice1" select="'0'"/>
            <xsl:with-param name="Indice2" select="'0'"/>
          </xsl:call-template>
         </OBR.4>
   <!--      <xsl:value-of select="/cmf:Message/cmf:Orders/cmf:Order/cmf:Procedure"/></OBR.4> -->
 <!--        <OBR.4><xsl:value-of select="/cmf:Message/cmf:Test/cmf:UserDefinedField1Value"/></OBR.4> -->
 <!--          <OBR.4.1>
        ...
        <OBR.16>
        <!--   <xsl:value-of select="/cmf:Message/cmf:Orders/cmf:Order/cmf:OrderPlacer"/> --> <!-- LM 05122017 Retirado a pedido Sérgio silva HLUZ -->
        </OBR.16>
		<OBR.20>
		<!-- 	<xsl:value-of select="/cmf:Message/cmf:Test/cmf:UserDefinedField3Value"/> -->
     <xsl:call-template name="FunctionESCOLHE_HLUZ"> 
            <xsl:with-param name="campo" select="/cmf:Message/cmf:Test/cmf:UserDefinedField3Value"/>      
            <xsl:with-param name="procedure1" select="/cmf:Message/cmf:Orders/cmf:Order/cmf:Procedure"/>
            <xsl:with-param name="Delimitador" select="'&#46;'"/>
		...
		<OBR.25>F</OBR.25> <!-- [LMM] 12102018 - Alteração do estado do relatório para F (final) -->
<!--         <OBR.25>
		...
		<OBR.34> <!-- LM 21112017 - Teste HLUZ para output do ID do Médico que valida o relatório  -->
          <OBR.34.1><xsl:value-of select="/cmf:Message/cmf:Reports/cmf:Report/cmf:InterpreterStaffNumber"/></OBR.34.1>
          <OBR.34.2><xsl:value-of select="/cmf:Message/cmf:Reports/cmf:Report/cmf:InterpreterFamilyName"/></OBR.34.2>
          <OBR.34.3><xsl:value-of select="/cmf:Message/cmf:Reports/cmf:Report/cmf:InterpreterGivenName"/></OBR.34.3>
          <OBR.34.6><xsl:value-of select="/cmf:Message/cmf:Reports/cmf:Report/cmf:InterpreterTitle"/></OBR.34.6>
        </OBR.34>
      </OBR>
<!--       <OBX>
        <OBX.1>1</OBX.1>
 ...
 <!--        <OBX.5>
        ...
        </OBX.5> --> <!-- LMM 21112017 - Desactivado base64 --> 
<!--         <OBX.11>
          <xsl:call-template name="FunctionCmfReportStatusToHl7">
            <xsl:with-param name="interpreter" select="/cmf:Message/cmf:Reports/cmf:Report/cmf:Interpreter/text()"></xsl:with-param>
          </xsl:call-template>
      ...
      <OBX>
        <OBX.1>1</OBX.1>
        <OBX.2>RP</OBX.2>
   <!--      <OBX.3><xsl:value-of select="/cmf:Message/cmf:Test/cmf:UserDefinedField1Value"/></OBX.3> -->
		<OBX.3>
			    <xsl:call-template name="FunctionESCOLHE_HLUZ"> 
            <xsl:with-param name="campo" select="/cmf:Message/cmf:Test/cmf:UserDefinedField3Value"/>      
            <xsl:with-param name="procedure1" select="/cmf:Message/cmf:Orders/cmf:Order/cmf:Procedure"/>
...
<!--           <OBX.3.1>
        ...
        <OBX.5>
          <OBX.5.1>
            <xsl:call-template name="RenameReportFileLUZ">
		...
		<OBX.11>F</OBX.11> <!-- [LMM] 12102018 - Alteração do estado do relatório para F (final) -->
 <!--        <OBX.11>
          <xsl:call-template name="FunctionCmfReportStatusToHl7">
            <xsl:with-param name="interpreter" select="/cmf:Message/cmf:Reports/cmf:Report/cmf:Interpreter/text()"></xsl:with-param>
          </xsl:call-template>
   ...
   <!--  <xsl:for-each select="/cmf:Message/cmf:ArrayOfMeasurement/cmf:Measurement"> -->
       <!--  <OBX>
          ...
          <OBX.3><xsl:value-of select ="cmf:Name"/></OBX.3>
          <OBX.5>
          ...
          <OBX.11><xsl:value-of select="cmf:Status"/></OBX.11>
        </OBX>  -->
   <!--    </xsl:for-each> -->
    </Hl7XmlMessage>
  </xsl:template>
