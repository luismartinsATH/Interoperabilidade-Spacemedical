...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:utils="urn:CodeUtils">
 ...
 <xsl:template name="RenameReportFileLUZ" xmlns:cmf="urn:dmr-cmf">
    <xsl:param name="CurrentName"/>
    <xsl:variable name="givenName" select="/cmf:Message/cmf:Patient/cmf:GivenName"/> 
	<xsl:variable name="IDprescricao" select="/cmf:Message/cmf:Orders/cmf:Order/cmf:PlacerNumber"/>
    <xsl:variable name="patientId" select="/cmf:Message/cmf:Patient/cmf:PatientID/cmf:PatientNumber"/>
    <xsl:variable name="arguments" select="concat($givenName, ',', $patientId,  ',' , substring-before($IDprescricao, '^'))"/>
    <xsl:value-of select="utils:RenamePdf($CurrentName, '{2}.pdf', $arguments)" />
  </xsl:template>
  ...
  <xsl:template name="FunctionVerificaCodigoExame">
    ...
    <xsl:template name="FunctionESCOLHE_HLUZ" xmlns:cmf="urn:dmr-cmf">
    <xsl:param name="campo"/>
    <xsl:param name="procedure1"/>
    <xsl:param name="Delimitador"/>   
    <xsl:param name="Indice1"/>
    <xsl:param name="Indice2"/>    
 	  <xsl:choose>
        <xsl:when test="$campo">
           <xsl:value-of select="$campo"/>
        </xsl:when>
        <xsl:otherwise>
          <xsl:call-template name="SelectToken"> <!-- Devolve o cód de exame original do HIS, armazenado em PROCEDURE [LMM] 14042020 -->
            <xsl:with-param name="text" select="$procedure1"/>
            <xsl:with-param name="delimiter" select="$Delimitador"/>
            <xsl:with-param name="requiredIndex" select="$Indice1"/>
            <xsl:with-param name="currentIndex" select="$Indice2"/>
          </xsl:call-template>  
        </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
