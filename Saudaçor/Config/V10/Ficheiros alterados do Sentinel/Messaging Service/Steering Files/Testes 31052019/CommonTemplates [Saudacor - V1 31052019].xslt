...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:utils="urn:CodeUtils">
  ...
  <xsl:template name="ConvertPathToUrl">
    <xsl:param name="path"/>
    <xsl:variable name="relativePath" select="substring-after($path, '\SENTINEL$\')"/>
    <xsl:variable name="url" select="concat('http://srsaservv0017.srsa.local:8050/', $relativePath)"/>
