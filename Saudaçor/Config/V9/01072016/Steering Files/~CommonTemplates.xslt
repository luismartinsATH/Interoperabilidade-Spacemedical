...
<!--  <xsl:template name="RenameReportFile" xmlns:cmf="urn:dmr-cmf">
  ...
  <xsl:variable name="givenName" select="/cmf:Message/cmf:Patient/cmf:GivenName"/>
  <xsl:variable name="patientId" select="/cmf:Message/cmf:Patient/cmf:PatientID/cmf:PatientNumber"/>
    <xsl:variable name="arguments" select="concat($patientId, ',','' )"/>
  <xsl:value-of select="utils:RenamePdf($CurrentName, '{0}{1}.pdf', $arguments)"/>
  </xsl:template>
  ...
  <xsl:template name="FunctionSentinelToHl7Units">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$unit='Unspecified'"/>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template name="FunctionSentinelToHl7DataTypes">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$type='Unspecified'"/>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template name="ConvertMeasurementValueToHl7">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$valueType='StringData' or $valueType='Number'                  or $valueType='ShortInteger' or $valueType='Integer' or $valueType='LongInteger'                  or $valueType='DoublePrecision' or $valueType='FloatingPoint' or $valueType='ImpresarioDateTime'                 or $valueType='Unspecified'">
        <xsl:value-of select="$valueToConvert"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template name="FunctionHl7ToSentinelProcedure">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$procedure='C41010^MONITORIZACAO PRESSAO ARTERIAL DURANTE 24 HORAS,INCLUINDO GRAVACAO E ANALISE POR SCANNING'">blood pressure</xsl:when>  <!-- Alterado por LM -->
<!--      <xsl:when test="starts-with($procedure,'blood')">blood pressure</xsl:when>-->
 ...
 <!--     <xsl:when test="starts-with($procedure, 'e83f0')">11524-0</xsl:when> -->
      ...
      <xsl:when test="starts-with($procedure, '769c3')">C41010^MONITORIZACAO PRESSAO ARTERIAL DURANTE 24 HORAS,INCLUINDO GRAVACAO E ANALISE POR SCANNING</xsl:when>
  ...
  <xsl:template name="FunctionSentinelTestTypeToHl7CodeSet">
    ...
    <xsl:choose>
      ...
      <xsl:when test="starts-with($procedure, '769c3')"/>
      <xsl:when test="starts-with($procedure, '8d91e')"/>
      <xsl:when test="starts-with($procedure, 'a8d82')"/>
    </xsl:choose>
  </xsl:template>

<!-- função desenvolvida por Andy (SLHC) para retirar o ID do paciente -->
...
<xsl:template name="ExtractID_1">     
 ...
 <xsl:template name="ExtractID_2">
  ...
  <xsl:template name="FunctionToOutputPatientID" xmlns:cmf="urn:dmr-cmf">

         <xsl:param name="ExtractedPatientID" select="/cmf:Message/cmf:Patient/cmf:PatientID/cmf:PatientNumber2"/>

         <xsl:if test="contains(.,'SONHO')">
           <xsl:variable name="ID_1" select="substring-before($ExtractedPatientID, 'SONHO')"/>
           <xsl:variable name="ID_2" select="substring-before($ExtractedPatientID, 'NS')"/>
           <xsl:variable name="ID_2_2" select="substring($ID_2, string-length($ID_2) - '4', 5)"/>
           <xsl:variable name="ID_3" select="substring-after($ExtractedPatientID, 'SONHO')"/>
           <xsl:variable name="ID_2_3" select="substring($ID_3, 1, 2)"/>
           <xsl:variable name="PID_1" select="concat($ID_1, '^^^', $ID_2_2, '^', $ID_2_3)"/>
           <xsl:value-of select="$PID_1"/>
         </xsl:if>

         <xsl:if test="contains(.,'RNU')">
           <xsl:variable name="ID_4" select="substring-before($ExtractedPatientID, 'RNU')"/>
           <xsl:variable name="ID_4_2" select="substring-after($ID_4, '&#126;')"/>
           <xsl:variable name="ID_5" select="substring($ExtractedPatientID, string-length($ExtractedPatientID) - '5', 3)"/>
           <xsl:variable name="ID_6" select="substring-after($ExtractedPatientID, 'RNU')"/>
           <xsl:variable name="PID_2" select="concat('~', $ID_4_2, '^^^', $ID_5, '^', $ID_6)"/>
           <xsl:value-of select="$PID_2"/>
         </xsl:if>
...
<!-- FUNÇÃO ALTERNATIVA PARA DEVOLVER NUMERO DE PACIENTE SE NÃO EXISTIR ORDER -->
<!--<xsl:template name="FunctionToOutputPatientID" xmlns:cmf="urn:dmr-cmf">

              <xsl:param name="ExtractedPatientID" select="/cmf:Message/cmf:Patient/cmf:PatientID/cmf:PatientNumber2"/>

              <xsl:if test="contains($ExtractedPatientID,'SONHO')">
                <xsl:variable name="ID_1" select="substring-before($ExtractedPatientID, 'SONHO')"/>
                <xsl:variable name="ID_2" select="substring-before($ExtractedPatientID, 'NS')"/>
                <xsl:variable name="ID_2_2" select="substring($ID_2, string-length($ID_2) - '4', 5)"/>
                <xsl:variable name="ID_3" select="substring-after($ExtractedPatientID, 'SONHO')"/>
              ...
              <xsl:if test="contains($ExtractedPatientID,'RNU')">
                <xsl:variable name="ID_4" select="substring-before($ExtractedPatientID, 'RNU')"/>
                <xsl:variable name="ID_4_2" select="substring-after($ID_4, '&#126;')"/>
                <xsl:variable name="ID_5" select="substring($ExtractedPatientID, string-length($ExtractedPatientID) - '5', 3)"/>
                <xsl:variable name="ID_6" select="substring-after($ExtractedPatientID, 'RNUS')"/>
                <xsl:variable name="PID_2" select="concat('~', $ID_4_2, '^^^', $ID_5, '^', $ID_6)"/>
                <xsl:value-of select="$PID_2" />
              </xsl:if>

              <xsl:if test="$ExtractedPatientID = ''">
                <xsl:value-of select="/cmf:Message/cmf:Patient/cmf:PatientID/cmf:PatientNumber"/>
              </xsl:if>
