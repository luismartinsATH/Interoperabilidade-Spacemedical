  ...
  <xsl:template match="/">
    <Message>
      <Processing>
        ...
        <MessageType>ADR^A19</MessageType>
        <MessageID><xsl:value-of select="/Hl7XmlMessage/QRD/QRD.4"/></MessageID>
      </Processing>
      <Patient>
        <PatientID>
          <xsl:variable name="GivenName" select="/Hl7XmlMessage/PID/PID.5/PID.5.1"/>
          <xsl:choose>
            <xsl:when test="string-length($GivenName) > 0">
              <PatientNumber><xsl:value-of select="/Hl7XmlMessage/PID/PID.3"/></PatientNumber></xsl:when>
            <xsl:otherwise><PatientNumber></PatientNumber></xsl:otherwise>
          </xsl:choose>
          <PatientNumber2>
