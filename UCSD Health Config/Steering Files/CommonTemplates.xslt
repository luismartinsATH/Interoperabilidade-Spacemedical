...
<!--
 ...
 --> 
  ...
  <xsl:template name="RenameReportFile" xmlns:cmf="urn:dmr-cmf">
    ...
    <xsl:param name="CurrentName"/>
<!-- SDA SDA format for Avera -->
   <xsl:param name="Identifier"/>
    <xsl:variable name="patientId" select="/cmf:Message/cmf:Patient/cmf:PatientID/cmf:PatientNumber"/>
    <xsl:variable name="patientId2" select="/cmf:Message/cmf:Patient/cmf:PatientID/cmf:PatientNumber2"/>
    <xsl:variable name="patientId3" select="/cmf:Message/cmf:Patient/cmf:PatientID/cmf:PatientNumber3"/>
	<xsl:variable name="UserDefinedField1Value" select="/cmf:Message/cmf:Test/cmf:UserDefinedField1Value"/> 
			<!-- Folder Name to put Report for facility lms 10-11-2017 -->
     
<xsl:variable name="nowDateTime">
<xsl:call-template name="SelectNextN">
    <xsl:with-param name="stringin" select="$now"/>
    <xsl:with-param name="nlength" select="12"/>
<xsl:with-param name="nstart" select="1"/>
</xsl:call-template>
</xsl:variable>
...
<!-- SDA: trial code used to extract part of "Identifier" (MessageID) 
<xsl:variable name="messageId">
<xsl:call-template name="SelectNextN">
    <xsl:with-param name="stringin" select="$Identifier"/>
    <xsl:with-param name="nlength" select="16"/>
<xsl:with-param name="nstart" select="3"/>
</xsl:call-template>
</xsl:variable>
 -->
...
<!--    0000000000000000{0}  is used to space fill to make 30 char MRN/PatientID outbound name -->
   <xsl:variable name="arguments" select="concat($patientId,',',$Identifier,',',$patientId3,',',$patientId2,',',$Status,',',$birthDate,',',$Gender,',',$nowDateTime)"/>
<xsl:value-of select="utils:RenamePdf($CurrentName, '0000000000000000{0}{1}{2}{3}ZZOVHOLTMONITO{4} 00000{5}{6}{7}.pdf', $arguments)" />
  </xsl:template>
  
<!-- *************************** above added from current production  lms 9-26-2017   ********************************  -->  
  <!-- SDA SDA Select next N characters of a string -->
<xsl:template name="SelectNextN">

    <xsl:param name="stringin"/>
    <xsl:param name="nlength"/>
   <xsl:param name="nstart"/>


      <xsl:variable name="instring" select="$stringin"/>
      <xsl:variable name="lengthn" select="$nlength"/>
<xsl:variable name="startn" select="$nstart"/>

      <xsl:value-of select="substring($instring,$startn,$nlength)"/>

  </xsl:template>
  
 <!-- *************************** above added from current production  lms 9-26-2017   ********************************  --> 

<!-- SDA Template used to convert Status from full description to a space or code letter for Avera -->
  <xsl:template name="FunctionStatusToCode">
    <xsl:param name="status" />
    <xsl:choose>
      <xsl:when test="$status='Confirmed'">R</xsl:when>
      <xsl:when test="$status='ConfirmedAndAmended'">A</xsl:when>
  ...
  -->
  <!-- ******************************************************************************* -->
    <!-- Determines if the report is S (Confirmed) or ConfirmedAndAmended = CS .
  If the report has been signed, it is Confirmed, otherwise it is unverified. -->
  <!-- LMS testing added 8-17-2016 also added to CmftoOru.xml  -->
  <xsl:template name="FunctionCmfReportStatus">
    <xsl:param name="testreportstatus" />
    <xsl:choose>
      <xsl:when test="starts-with($testreportstatus,'ConfirmedAndAmended')">C</xsl:when>
	  <xsl:when test="starts-with($testreportstatus,'Confirmed')">F</xsl:when>
	  <xsl:when test="starts-with($testreportstatus,'Completed')">P</xsl:when>
	  <xsl:when test="starts-with($testreportstatus,'Unconfirmed')">P</xsl:when>
  ...
  <xsl:template name="FunctionSentinelToHl7DataTypes">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$type='Unspecified'">TX</xsl:when>
    </xsl:choose>
  </xsl:template>
...
<!-- ***   Order Control codes ORC.2.1 Sentinel by default supports NW, CA, XO the below tries to match up other control codes to supported codes    ***  -->

	<xsl:template name="FunctionOrderControlCodeSelector">
		<xsl:param name="ControlCode"/>
		<xsl:choose>
			<xsl:when test="$ControlCode='NW'">NW</xsl:when> <!-- NW = New order/service  -->
			<xsl:when test="$ControlCode='PA'">NW</xsl:when>
			
			<xsl:when test="$ControlCode='CA'">CA</xsl:when> <!-- Cancel order/service request -->
			<xsl:when test="$ControlCode='CR'">CA</xsl:when>
			<xsl:when test="$ControlCode='DC'">CA</xsl:when>
			<xsl:when test="$ControlCode='DR'">CA</xsl:when>
			<xsl:when test="$ControlCode='OC'">CA</xsl:when>
			<xsl:when test="$ControlCode='OD'">CA</xsl:when>
			
			<xsl:when test="$ControlCode='XO'">XO</xsl:when> <!-- Change order/service request -->
			<xsl:when test="$ControlCode='XR'">XO</xsl:when>
			<xsl:when test="$ControlCode='XX'">XO</xsl:when>
			<xsl:when test="$ControlCode='RU'">XO</xsl:when>			
			<xsl:when test="$ControlCode='RQ'">XO</xsl:when>
			<xsl:when test="$ControlCode='RP'">XO</xsl:when>
			<xsl:when test="$ControlCode='RO'">XO</xsl:when>
			</xsl:choose>
	</xsl:template>
<!--   ORM Config file org. Setting below now should use the function above. LMS Aug. 15 2018 -->
<!--
	      <ControlCode>
...
-->  
  ...
  <xsl:template name="FunctionSelectOrganisation">
    <xsl:choose>
      <xsl:when test="/Hl7XmlMessage/MSH/MSH.3/MSH.3.1">   <!--  Organisation location = ?  MSH.2 --> 
        <xsl:value-of select="/Hl7XmlMessage/MSH/MSH.3/MSH.3.1"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:value-of select="/Hl7XmlMessage/MSH/MSH.3"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>  
  ...
  <xsl:template name="FunctionSelectPatientId">
    <xsl:choose>
      <xsl:when test="/Hl7XmlMessage/PID/PID.3/PID.3.1">   <!--  MRN PID.3 --> 
        <xsl:value-of select="/Hl7XmlMessage/PID/PID.3/PID.3.1"/> <!-- ^<xsl:value-of select="/Hl7XmlMessage/PID/PID.3/PID.3.2"/>^<xsl:value-of select="/Hl7XmlMessage/PID/PID.3/PID.3.3"/>^<xsl:value-of select="/Hl7XmlMessage/PID/PID.3/PID.3.4"/>-->
   ...
   <xsl:template name="FunctionSelectPatientId2">
    <xsl:choose>
      <xsl:when test="/Hl7XmlMessage/PID/PID.2/PID.2.1"> <!--    Secondary MRN in PID.2 --> 
        <xsl:value-of select="/Hl7XmlMessage/PID/PID.2/PID.2.1"/>^<xsl:value-of select="/Hl7XmlMessage/PID/PID.2/PID.2.2"/>^<xsl:value-of select="/Hl7XmlMessage/PID/PID.2/PID.2.3"/>^<xsl:value-of select="/Hl7XmlMessage/PID/PID.2/PID.2.4"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:value-of select="/Hl7XmlMessage/PID/PID.2"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template> 

   <xsl:template name="FunctionSelectPatientId3">
    <xsl:choose>
      <xsl:when test="/Hl7XmlMessage/PID/PID.18/PID.18.1"> <!--   Patient Account Number  in PID.18 --> 
        <xsl:value-of select="/Hl7XmlMessage/PID/PID.18/PID.18.1"/>^<xsl:value-of select="/Hl7XmlMessage/PID/PID.18/PID.18.2"/>^<xsl:value-of select="/Hl7XmlMessage/PID/PID.18/PID.18.3"/>^<xsl:value-of select="/Hl7XmlMessage/PID/PID.18/PID.18.4"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:value-of select="/Hl7XmlMessage/PID/PID.18"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template> 

   <xsl:template name="FunctionSelectVisitNumber">
    <xsl:choose>
      <xsl:when test="/Hl7XmlMessage/PV1/PV1.19/PV1.19.1"> <!--   Patient Account Number  in PV1.19 --> 
        <xsl:value-of select="/Hl7XmlMessage/PV1/PV1.19/PV1.19.1"/>^<xsl:value-of select="/Hl7XmlMessage/PV1/PV1.19/PV1.19.2"/>^<xsl:value-of select="/Hl7XmlMessage/PV1/PV1.19/PV1.19.3"/>^<xsl:value-of select="/Hl7XmlMessage/PV1/PV1.19/PV1.19.4"/>
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:template name="GetCasesToMerge">
	...
	<!-- looks for NTE & DG1 segments in HL7 fields, then if multiples are found concatantate together and return results typically save to the Test Comments -->

	<xsl:template name="FunctionSelectNTEFields">
		<xsl:for-each select="/Hl7XmlMessage/node()">
		...
		<xsl:for-each select="/Hl7XmlMessage/node()"> 
			<xsl:value-of select="DG1.3/DG1.3.1"/>, <xsl:value-of select="DG1.3/DG1.3.2"/>, <xsl:value-of select="DG1.3/DG1.3.3"/>, <xsl:value-of select="DG1.4"/>, <xsl:value-of select="DG1.6"/>
		</xsl:for-each>
	</xsl:template>

	<!-- ************************************************************************************************************************************************************* -->

   <!--  testing  ******************* larry split nodes() -->
       <xsl:template name="line">  
        <xsl:param name="Comments" />  
        <xsl:choose>  
            <xsl:when test="not(contains($Comments, '&#xA;'))">  
                <OBX.5.1>  
                <xsl:copy-of select="concat($Comments, '\.br\')"/> 
                </OBX.5.1>  
            </xsl:when>  
            <xsl:otherwise>  
                <OBX.5.1>  
                ...
                <xsl:call-template name="line">  
                    <xsl:with-param name="Comments" select="substring-after($Comments, '&#xA;')"/>   
                </xsl:call-template>  
            </xsl:otherwise>  
        </xsl:choose>  
 ...
 <!--  testing  *******************  -->	
  ...
  <xsl:template name="substring-after-last">
   ...
   <!--    **********  FROM LomondMessagingService codes   *****  LMS 10-26-2016 added the below script to handle Site Procedure Codes -->
    <!-- Convert from the Universal Service ID (HL7) to a procedure that Sentinel understands. FunctionHl7ToSentinelProcedure  -->
    ...
    <xsl:choose>
      ...
      <xsl:when test="starts-with($procedure,'Holter Monitor 24')">18754-2^LN</xsl:when>	 <!--  Custom Procedure codes -->
		...
		<!--    **********  FROM LomondMessagingService codes   *****  LMS 8-26-2018 
				...
				CARD15^AMB 24 HR BLOOD PRESSURE MONITOR^SENTINEL^^AMB 24 HR BLOOD PRESSURE MONITOR
		-->
