...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="urn:dmr-cmf" xmlns:utils="urn:CodeUtils">
  ...
  <xsl:template match="/">
    <Message>
      ...
      <Patient>
        <PatientID>
          <Organisation>UCSD</Organisation>
        ...
        <Visit>
		  ...
		  <!--    FunctionSelectVisitNumber
          <VisitNumber>
            <xsl:value-of select="/Hl7XmlMessage/PV1/PV1.19"/>
          </VisitNumber>
		  -->
          <VisitNumber><xsl:call-template name="FunctionSelectVisitNumber"/></VisitNumber>
      ...
      <Orders>
        <Order>
			<!--  using the below function to support more control codes for Orders. Aug. 15 2018 lms -->
 		  <ControlCode>
				<xsl:call-template name="FunctionOrderControlCodeSelector">
				  <xsl:with-param name="ControlCode">
		...
		-->
          <PlacerNumber>
			...
			<Procedure>
					  <xsl:value-of select="/Hl7XmlMessage/OBR/OBR.4/OBR.4.1"/>^<xsl:value-of select="/Hl7XmlMessage/OBR/OBR.4/OBR.4.2"/>^<xsl:value-of select="/Hl7XmlMessage/OBR/OBR.4/OBR.4.3"/>^<xsl:value-of select="/Hl7XmlMessage/OBR/OBR.4/OBR.4.4"/>^<xsl:value-of select="/Hl7XmlMessage/OBR/OBR.4/OBR.4.5"/>
            </Procedure> <!-- CARD7^HOLTER MONITOR (24 HOURS)^SENTINEL^^HOLTER MONITOR  -->

	
			<TestType>
				<xsl:call-template name="FunctionHl7ToSentinelProcedure">    <!-- CommonTemplates   -->
        ...
        of the ordering provider as this will be the form it is returned in report messages. -->
          <OrderPlacer>
			 <xsl:value-of select="/Hl7XmlMessage/OBR/OBR.16/OBR.16.2"/>^<xsl:value-of select="/Hl7XmlMessage/OBR/OBR.16/OBR.16.3"/>^<xsl:value-of select="/Hl7XmlMessage/OBR/OBR.16/OBR.16.4"/>
          </OrderPlacer>
          <Notes>
