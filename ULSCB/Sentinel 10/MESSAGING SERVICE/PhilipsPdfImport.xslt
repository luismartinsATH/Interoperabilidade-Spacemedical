...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="urn:dmr-cmf" xmlns:utils="urn:CodeUtils">
    ...
    <Message>
      ...
      <!--     <xsl:call-template name="ExtractPatientFamilyName"/> -->
        ...
        <Location>
          ...
          <Ward></Ward>
          <Bed>
			<xsl:call-template name="ExtractProcesso"/>
		  </Bed>
          <Facility></Facility>
        </Location>
      </Patient>
      <Test>
        <StartDateTime><xsl:call-template name="ExtractRecordingDateTime"/></StartDateTime>        <!--    -->
        <RecordingDuration></RecordingDuration>
        <RecorderModel>CardioExpress</RecorderModel>
      ...
      <Reports>
        <Report>
          ...
          <TestType>resting</TestType> <!--             <xsl:call-template name="ExtractTestType"/> -->
          <RecordingDateTime></RecordingDateTime>
          <ReportFormat></ReportFormat>
          <ReportType>resting</ReportType> <!-- <xsl:call-template name="ExtractTestType"/> -->
  ...
  <xsl:template name="ExtractPatientFamilyName">
    ...
    <xsl:call-template name="SelectToken">
      ...
      <xsl:with-param name="requiredIndex">2</xsl:with-param>  <!--  [LM] alterado para 2 -->
  ...
  In: ZH_101[20050516+1030]Rufus_T_Firefly.pdf 
  Out: Rufus -->
   <!--  [LM] Alteração de função que reconhece Nome de paciente -->
  ...
  In: ZH_101[20050516+1030]Rufus_T_Firefly.pdf 
  Out: 101 -->
 <!--  [LM] Alteração de função que reconhece ID -->
  <xsl:template name="ExtractPatientID">
    <xsl:variable name="filename">
      <xsl:call-template name="GetReportFilename"/>
    </xsl:variable>
	 <xsl:variable name="filenameWithoutExtension" select="substring-before($filename, '[')"/>
	 <xsl:value-of select="substring-after($filenameWithoutExtension, '-')"/>
   <!--  <xsl:value-of select="substring-before($filename, '_')"/>  --> <!-- LMM 14052018 Alteração para extrair ID de ORG_ID -->
  </xsl:template>
  
  <xsl:template name="ExtractProcesso">
    <xsl:variable name="filename">
      <xsl:call-template name="GetReportFilename"/>
    </xsl:variable>
    <xsl:value-of select="substring-before($filename, '-')"/>  <!-- LMM 14052018 Alteração para extrair ORG de ORG_ID -->
  </xsl:template>
